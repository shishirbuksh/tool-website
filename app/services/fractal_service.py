...
class FractalService:
    ...
    async def generate_nft(self, prompt: str, style: str = "3d", provider: str = "local", api_key: str | None = None) -> dict[str, Any]:
        ...
        prompt = prompt.strip()
        if len(prompt) > 1000:
            raise ValidationException("Prompt exceeds 1000 character limit")
        ...
        if provider == "local":
            ...
            # Daily bucket for cacheability (was //10 — nondeterministic within minutes).
            h = hashlib.sha256(f"{prompt}_{style}_{int(time.time() // 86400)}".encode()).hexdigest()
    ...
    async def _generate_pattern(
        ...
        loop = asyncio.get_running_loop()
        sem = _get_fractal_semaphore()
        try:
            await asyncio.wait_for(sem.acquire(), timeout=10.0)
        except TimeoutError:
            raise ServiceError("Fractal engine busy — too many concurrent renders") from None

        def _gen():
            return rust_predictor.generate_pattern(width, height, zoom, c_re, c_im, max_iter)

        try:
            return await asyncio.wait_for(loop.run_in_executor(None, _gen), timeout=120)
        except TimeoutError:
